...
def train_quantum_model(X_train, y_train, X_val, y_val, epochs=15):
    model     = get_model()
...
def train_classical_baselines(X_train, y_train, X_test, y_test):
    ...
    with open("processed/svm.pkl", "wb") as f:
        pickle.dump(svm, f)
    ...
    with open("processed/rf.pkl", "wb") as f:
        pickle.dump(rf, f)
    return results
if __name__ == "__main__":
    X_train, X_val, X_test, y_train, y_val, y_test = load_data()
    print("Training Hybrid QML model (Qiskit)...")
    print("Estimated time: 30-45 minutes with optimized settings")
    model, history = train_quantum_model(
    ...
    for name, r in all_results.items():
        print(f"{name}: Acc={r['accuracy']:.4f} | "
              f"F1={r['f1']:.4f} | FNR={r['false_neg_rate']:.4f}")
