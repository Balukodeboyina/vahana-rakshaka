...
with tab3:
    st.subheader("The Variational Quantum Circuit (VQC) — Qiskit")
    st.markdown("""
    This is the quantum core of Vahana Rakshaka built with **Qiskit**.
    The circuit encodes CAN bus features as rotation angles on 4 qubits,
    then applies entangling layers to detect attack signatures as quantum states.
    """)
    try:
        qc = get_circuit_diagram()
        fig, ax = plt.subplots(figsize=(12, 4))
        qc.draw(output="mpl", ax=ax, style="clifford")
        plt.tight_layout()
        st.pyplot(fig)
        plt.close()
    except Exception as e:
        st.info(f"Circuit diagram: {e}")

    st.markdown("---")
    st.subheader("How it works — step by step")

    with st.expander("Step 1: Feature encoding via Rx (AngleEmbedding)"):
        st.markdown("""
    ...
    with st.expander("Step 2: Rz-Ry-Rz + CNOT Entangling Layers"):
        st.markdown("""
        3 layers of parameterized **Rz-Ry-Rz** rotation gates + **CNOT** entanglement gates.
    ...
    with st.expander("⚛️ Why Qiskit?"):
        st.markdown("""
        **Qiskit** is IBM's open-source quantum computing framework — the most widely used
        in industry and research. Using Qiskit means:

        - ✅ Direct compatibility with **IBM Quantum** real hardware
        - ✅ Can run on **127+ qubit** IBM Eagle processors
        - ✅ Industry standard for quantum ML deployment
        - ✅ Supports **Amazon Braket** and other cloud quantum APIs
        """)
with tab4:
